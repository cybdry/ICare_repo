...
def get_api_wrappper(audio):
    ...
    if key:
        openai_output = chatgpt_api(get_user[0])

    if len(openai_output) != 0:
        translated = translator.translate(openai_output,src="en",dest="es")
        audio_file_spanish = convert_text_to_audio(translated,"es","openai_output.mp3")
    else:
        translated = translator.translate(get_user[1],src="en",dest="es")
        audio_file_spanish = convert_text_to_audio(translated,"es","openai_output.mp3")
